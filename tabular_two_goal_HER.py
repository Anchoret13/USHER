	...
	def update(self, episode: list, lr: float=.05) -> None:
		...
		for i in range(len(episode)): 
			...
			# for _ in range(i, k):
				...
				self._update_usher_q(state, goal, desired_goal, action, next_state, reward, lr, p=self.p + gamma**(i+1))
...
def learn_q_function(k: int = 4):
	...
	for episode in range(episodes):
		...
		power = .75
		ep_scale = 30
		lr = (2**(-ep_scale*episode/episodes) + base_lr/(episodes**power/100+1))
		...
		if episode%record_num == 0: 
			...
			ave_lr = (2**(-ep_scale*episode/episodes) + .5/(episodes**power/100+1))
