...
from gridworld2 import create_map_1
...
class Q: 
	def __init__(self, size: int, compute_reward, default_goal: np.ndarray, k: int = 4):
		...
		self.pure_q_table = np.ones((env.size, env.size, 5)) + 1
	...
	def _update_pure_q(self, state: np.ndarray, action: np.ndarray, next_state: np.ndarray, reward: float, lr: float=.05) -> None:
	...
	def update(self, episode: list, lr: float=.05) -> None:
		...
		for i in range(len(episode)): 
			...
			self._update_pure_q(state, desired_goal, action, next_state, lr)
