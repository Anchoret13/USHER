    ...
    for i in range(0, grid.shape[0]):
        for j in range(0, grid.shape[1]):
            ...
            if block_type == BLOCK: 
                ...
                pygame.draw.rect(SCREEN, color, rect, 0)
                if block_type == BREAKING_DOOR or block_type == LOWCHANCE_BREAKING_DOOR:
...
if __name__ == "__main__":
    import alt_gridworld_implementation
    env = alt_gridworld_implementation.random_map()
    q = object()
    draw_init(env, q)
    while True:
        draw_grid(env, q)