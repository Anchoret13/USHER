...
from mpi_utils.mpi_utils import sync_networks, sync_grads #type: ignore
from mpi_utils.normalizer import normalizer 
from rl_modules.replay_buffer import replay_buffer #type: ignore
...
from rl_modules.usher_models import test_T_conditioned_ratio_critic as critic
from rl_modules.alternate_sac_actor import actor #type: ignore
from her_modules.usher import her_sampler #type: ignore
import pdb
...
class ddpg_agent:
    ...
    # def _preproc_inputs(self, obs: ObsArray, g: GoalArray, gpi: Optional[GoalArray] = None) -> Tensor:
        #This is for checking to see if the typechecker is properly detecting type mismatches
    def _preproc_inputs(self, obs: Array, g: Array, gpi: Optional[Array] = None) -> NormedTensor:
        obs_norm = self.o_norm.normalize(obs)
        g_norm = self.g_norm.normalize(g)
        # concatenate the stuffs
        gpi_norm: list = [] if type(gpi) == type(None) else [self.g_norm.normalize(gpi)]#type: ignore
        # gpi_norm: list = [] if isinstance(gpi, type(None)) else [self.g_norm.normalize(gpi)]
        npinputs = np.concatenate([obs_norm, g_norm] + gpi_norm) #type: ignore
    ...
    def _select_actions(self, pi: Tensor) -> Array:
    ...
    def _preproc_og(self, o: Array, g: Array) -> Tuple[Array, Array]:
        clipped_o = Array(np.clip(o, -self.args.clip_obs, self.args.clip_obs))
        clipped_g = Array(np.clip(g, -self.args.clip_obs, self.args.clip_obs))
