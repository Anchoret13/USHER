...
def softmax(arr, temp: float) -> Tuple[float]:
	# unnormed_vals = 2**(temp*arr)
	unnormed_vals = tuple(2**(temp*a) for a in arr)
...
class Q: 
	def __init__(self, size: int, compute_reward, default_goal: Goal, k: int = 4):
	...
	def sample_action(self, state: State, goal: Goal, pol_goal: Goal =None, temp: float=5) -> int:
		use_pol_goal: Goal = goal if type(pol_goal == None) else pol_goal
		loc: tuple = state + goal + use_pol_goal
		q = self.q_table[loc]
		samp = softmax_sample(q, temp)
		return samp

	# def argmax_action(self, state: State, goal: Goal, pol_goal: Goal | None =None, temp: float=5) -> int:
	def argmax_action(self, state: State, goal: Goal, pol_goal: Goal =None, temp: float=5) -> int:
		use_pol_goal = goal if type(pol_goal == None) else pol_goal
		q = self.q_table[state + goal + use_pol_goal]
		return q.argmax()
	...
	def _update_usher_q(self, state: State, goal: Goal, pol_goal: Goal, action: ActionIndex, 
			# next_state: State, reward: float, lr: float=.05,  p: float | None=None, t_remaining=None) -> None:
			next_state: State, reward: float, lr: float=.05,  p: float=None, t_remaining=None) -> None:
